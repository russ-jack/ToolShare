...
import { getDB, saveDB, getTools, getCurrentUser, loginUser, logoutUser, addTool, fetchToolsFromAPI, deleteToolBackend, updateToolBackend } from './services/db.js';
        ...
        tools.forEach(tool => {
            ...
            tr.innerHTML = `
                ...
                <td class="px-4 py-3 text-right flex justify-end gap-1">
                    <button ${tool.status !== 'available' ? 'disabled' : `onclick="editTool(${tool.id})"`} class="${tool.status !== 'available' ? 'opacity-30 cursor-not-allowed text-on-surface-variant' : 'text-on-surface-variant hover:text-primary transition-colors'} p-2 rounded-full" title="${tool.status !== 'available' ? 'No puedes editar porque está prestado' : 'Editar'}"><span class="material-symbols-outlined text-[18px]">edit</span></button>
                    <button ${tool.status !== 'available' ? 'disabled' : `onclick="deleteTool(${tool.id})"`} class="${tool.status !== 'available' ? 'opacity-30 cursor-not-allowed text-error' : 'text-error hover:bg-error/10 transition-colors'} p-2 rounded-full" title="${tool.status !== 'available' ? 'No puedes eliminar porque está prestado' : 'Eliminar'}"><span class="material-symbols-outlined text-[18px]">delete</span></button>
                </td>
            `;
...
window.editTool = function(id) {
    ...
    showAddToolModal(async (updatedData) => {
        ...
        await updateToolBackend(id, newData);
        renderDashboardTools();
